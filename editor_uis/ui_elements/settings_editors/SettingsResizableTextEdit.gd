...
func _ready() -> void:
	if eh_DocsExporterPlugin.is_current_edited_scene(self) or _property == "none":
		return
...
func _on_SettingsResizableTextEdit_text_changed():
	_settings.set(_property, text)
...
func _get_possible_string_properties() -> PoolStringArray:
	...
	for property_dict in settings_properties:
		...
		if property_dict.type == TYPE_STRING and property_dict.hint == PROPERTY_HINT_MULTILINE_TEXT:
